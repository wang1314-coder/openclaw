...
export const FIELD_HELP: Record<string, string> = {
  ...
  "cron.modelPreflight.maxAttempts":
    "Number of local model-provider preflight attempts before cron advances to a configured fallback or marks the run skipped (default: 1). Increase to let sleeping Ollama/vLLM/LM Studio hosts wake before cron gives up.",
  "cron.modelPreflight.retryDelayMs":
    "Delay in milliseconds between failed local model-provider preflight attempts (default: 0). Use with maxAttempts to create a warm-up window before cron advances to a fallback or skips the run.",
  transcripts:
