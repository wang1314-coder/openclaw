...
function normalizeIdentityToken(value: unknown): string | undefined {
  return normalizePromptMetadataString(value)?.toLowerCase();
}

function addScopedRequesterIdentityCandidate(params: {
  candidates: Array<{ token: string; matchedBy: TrustedRequesterIdentity["matched_by"] }>;
  seen: Set<string>;
  channel?: string;
  value: unknown;
  matchedBy: TrustedRequesterIdentity["matched_by"];
}): void {
  const channel = normalizeIdentityToken(params.channel);
  const value = normalizeIdentityToken(params.value);
  if (!channel || !value) {
    return;
  }
  const token = `${channel}:${value}`;
  if (params.seen.has(token)) {
    return;
  }
  params.seen.add(token);
  params.candidates.push({ token, matchedBy: params.matchedBy });
}

function resolveTrustedRequesterIdentity(params: {
  ctx: TemplateContext;
  identityLinks?: Record<string, string[]>;
}): TrustedRequesterIdentity | undefined {
  const identityLinks = params.identityLinks;
  if (!identityLinks || typeof identityLinks !== "object") {
    return undefined;
  }
  const channel = resolveInboundChannel(params.ctx);
  const candidates: Array<{
    token: string;
    matchedBy: TrustedRequesterIdentity["matched_by"];
  }> = [];
  const seen = new Set<string>();
  addScopedRequesterIdentityCandidate({
    candidates,
    seen,
    channel,
    value: params.ctx.NativeDirectUserId,
    matchedBy: "native_direct_user_id",
  });
  addScopedRequesterIdentityCandidate({
    candidates,
    seen,
    channel,
    value: params.ctx.SenderId,
    matchedBy: "sender_id",
  });
  addScopedRequesterIdentityCandidate({
    candidates,
  ...
  for (const [canonical, ids] of Object.entries(identityLinks)) {
    const canonicalId = normalizePromptMetadataString(canonical);
    if (!canonicalId || !Array.isArray(ids)) {
      continue;
    }
    for (const id of ids) {
      const normalizedId = normalizeIdentityToken(id);
      if (!normalizedId) {
        continue;
      }
      const match = candidates.find((candidate) => candidate.token === normalizedId);
      if (match) {
        return {
...
export function buildInboundMetaSystemPrompt(
  ctx: TemplateContext,
  options?: { includeFormattingHints?: boolean; identityLinks?: Record<string, string[]> },
): string {
  ...
  const requesterIdentity = resolveTrustedRequesterIdentity({
    ctx,
  ...
  return [
    ...
    ...(requesterIdentity
      ? [
          "requester_identity is resolved from stable provider ids matched against trusted configuration such as session.identityLinks, not from display names or quoted text.",
        ]
      : []),
