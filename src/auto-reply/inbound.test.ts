...
describe("flushAllInboundDebouncers", () => {
  ...
  it("keeps active debouncers registered after a global flush", async () => {
    vi.useFakeTimers();
    const calls: Array<string[]> = [];
  ...
  it("passes internal gateway-drain context to global flush handlers", async () => {
    vi.useFakeTimers();
    const contexts: Array<{ reason?: string; allowDuringGatewayDrain?: boolean }> = [];

    const debouncer = createInboundDebouncer<{ key: string; id: string }>({
      debounceMs: 5000,
      buildKey: (item) => item.key,
      onFlush: async (_items, context) => {
        contexts.push({
          reason: context?.reason,
          allowDuringGatewayDrain: context?.allowDuringGatewayDrain,
        });
      },
    });

    await debouncer.enqueue({ key: "session-1", id: "msg-1" });
    expect(await flushAllInboundDebouncers()).toBe(1);
    expect(contexts).toEqual([{ reason: "flush-all", allowDuringGatewayDrain: true }]);

    vi.useRealTimers();
  });

  it("allows restart drain context to flush handlers that enqueue command work", async () => {
    ...
    const debouncer = createInboundDebouncer<{ key: string; id: string }>({
      ...
      onFlush: async (items) => {
        for (const item of items) {
          await enqueueCommandInLane(`inbound-debounce-test:${item.key}`, async () => {
    ...
    await expect(flushAllInboundDebouncers()).resolves.toBe(1);
    expect(delivered).toEqual(["msg-1"]);
    expect(errors).toEqual([]);
  });

  it("keeps quiet debouncers registered across restart-drain sweeps", async () => {
    vi.useFakeTimers();
    const calls: Array<string[]> = [];
    ...
    // Advance past the old staleness window with no pending message. The
    // debouncer still represents a live channel until teardown unregisters it.
...
describe("createInboundDebouncer flushAll", () => {
  it("preserves flushKey void contract and exposes counts separately", async () => {
    vi.useFakeTimers();
    const calls: Array<string[]> = [];
    ...
    await debouncer.enqueue({ key: "a", id: "1" });
    await expect(debouncer.flushKey("a")).resolves.toBeUndefined();
    await debouncer.enqueue({ key: "b", id: "2" });
    await expect(debouncer.flushKeyWithCount("b")).resolves.toEqual({ flushed: 1 });
    await expect(debouncer.flushKeyWithCount("missing")).resolves.toEqual({ flushed: 0 });
    expect(calls).toEqual([["1"], ["2"]]);
