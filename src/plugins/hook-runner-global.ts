...
function composeLiveHookRegistry(
  ...
  // Prefer a source where the plugin actually loaded, so a failed or disabled
  // record from a later scoped reload cannot shadow a healthy pinned
  // registration (which would silently drop a fail-closed tool-call gate).
