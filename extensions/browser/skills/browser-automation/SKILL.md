...
3. Read before you click:
   ...
   - Use `labels=true` on snapshot or screenshot when visual position matters. The response includes an `annotations` array (`{ref, number, role, name?, box}`) with each ref's bounding box in the captured image's coordinate space, so you can reason about position without re-snapshotting. Combine with `fullPage=true` (CLI: `--full-page`) to label the whole document, or `ref` / `element` to clip to one element. The `annotations` array is only returned by the Playwright driver; `profile="user"` and other existing-session profiles, plus the raw-CDP fallback, render the overlay into the screenshot but do not attach `annotations`, so on those profiles read positions from the labeled image itself rather than expecting the array.
4. Act narrowly:
