...
type ApplyShortTermPromotionsOptions = {
  ...
  timezone?: string;
  /**
   * Deprecated compatibility knob from the former root-MEMORY promotion flow.
   * Detailed promotion excerpts are now written to archives, leaving MEMORY.md
   * with only a compact pointer section, so this option is ignored.
   */
  memoryFileMaxChars?: number;
  /**
   * Deprecated compatibility knob from the former root-MEMORY promotion flow.
   * Archives preserve full excerpts, so this value is accepted but ignored.
   */
...
export function normalizeShortTermRecallStore(raw: unknown, nowIso: string): ShortTermRecallStore {
  ...
  if (entriesRaw && typeof entriesRaw === "object") {
    for (const [key, rawEntry] of Object.entries(entriesRaw as Record<string, unknown>)) {
...
export async function recordDreamingPhaseSignals(params: {
  ...
  await withShortTermLock(workspaceDir, async () => {
    ...
    for (const key of keys) {
      ...
      const entry: ShortTermPhaseSignalEntry = phaseSignals.entries[key] ?? {
        key,
        lightHits: 0,
        remHits: 0,
      };
...
function buildPromotionPointerSection(archiveRelativePath: string): string {
  ...
  return [
    ...
    "",
    "- Detailed promoted-memory excerpts are archived under `memory/archived/YYYY-Q#/memory-promoted-short-term-dump-YYYY-MM-DD.md`.",
    "- Keep `MEMORY.md` index-like; use the original daily memory files and `memory/categories/` for searchable detail.",
...
async function collectArchivedPromotionMarkerLocations(
  ...
  for (const quarter of quarters
    ...
    for (const file of files
      ...
      const text = await fs.readFile(path.join(quarterDir, file.name), "utf-8").catch((err: unknown) => {
        if ((err as NodeJS.ErrnoException)?.code === "ENOENT") {
          return "";
        }
        throw err;
