...
export function createSignalEventHandler(deps: SignalEventHandlerDeps) {
  ...
  async function handleSignalInboundMessage(entry: SignalInboundEntry) {
    ...
    const ctxPayload = buildChannelInboundEventContext({
      channel: "signal",
      supplemental: {
        // Surface the inbound quote-reply target so finalizeCoreInboundContext
        // maps it into ReplyToId/ReplyToBody/ReplyToSender/ReplyToIsQuote.
        quote: entry.quoteTarget
          ? {
              id: entry.quoteTarget.id,
              body: entry.quoteTarget.body,
              sender: entry.quoteTarget.author,
              isQuote: true,
            }
          : undefined,
      },
      ...
      message: {
        body: combinedBody,
        // Fall back to the quoted body for quote-only messages so the agent
        // still receives the reply context when no new text was sent.
        bodyForAgent,
        inboundHistory,
