...
describe("wrapCopilotAnthropicStream", () => {
  it("adds Copilot headers, strips Anthropic thinking replay, and marks cache for Claude payloads", () => {
  ...
  it("replaces thinking-only assistant replay messages for Copilot Claude payloads", () => {
    const payloads: Array<{
      messages: Array<Record<string, unknown>>;
    }> = [];
    const baseStreamFn = vi.fn((model, _context, options) => {
