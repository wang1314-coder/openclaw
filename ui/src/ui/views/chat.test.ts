...
describe("chat attachment picker", () => {
  ...
  it("converts large pasted data image text without full-string regex capture", () => {
    // Pins index-based parsing of the pasted body; the replaced full-string
    // regex capture could exhaust the regex stack on multi-megabyte pastes.
    const onAttachmentsChange = vi.fn();
    const container = renderChatView({ onAttachmentsChange });
    const textarea = requireElement(
      container,
  ...
  it("ignores pasted data URLs with non-base64 payload characters", () => {
    ...
    Object.defineProperty(event, "clipboardData", {
      value: {
        items: { length: 0 },
        getData: (type: string) => (type === "text/plain" ? "data:image/png;base64,AA$A" : ""),
      },
    });
