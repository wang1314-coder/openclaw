...
function createChatHeaderState(
  ...
  const state = {
    ...
    chatSessionPickerEditingKey: null,
    chatSessionPickerEditingValue: "",
    chatSessionPickerSavingKey: null,
...
describe("chat session controls", () => {
  ...
  it("opens inline rename controls for picker rows", async () => {
    const { state } = createChatHeaderState();
    state.sessionKey = "agent:main:main";
    state.settings.sessionKey = state.sessionKey;
    state.chatSessionPickerOpen = true;
    ...
    expect(state.chatSessionPickerEditingKey).toBe("agent:main:work");
    expect(input?.value).toBe("Main work");
  });

  it("renames picker sessions through sessions.patch and refreshes the list", async () => {
    const { state } = createChatHeaderState();
    state.sessionKey = "agent:main:main";
    state.settings.sessionKey = state.sessionKey;
    ...
    const request = vi.fn(async (method: string, params: Record<string, unknown> = {}) => {
      if (method === "sessions.patch") {
        ...
        const nextLabel =
          typeof params.label === "string" ? params.label : params.label === null ? null : null;
        rows = rows.map((row) =>
          row.key === key ? { ...row, label: nextLabel ?? undefined } : row,
        );
        return { ok: true, key };
      }
      if (method === "sessions.list") {
    ...
    await vi.waitFor(() => expect(state.chatSessionPickerEditingKey).toBeNull());

    render(renderChatSessionSelect(state), container);
    const labels = Array.from(
      container.querySelectorAll<HTMLElement>(".chat-session-picker__option-label"),
    ).map((node) => node.textContent?.trim());
    expect(labels).toContain("Ops notes");
    expect(state.sessionsResult?.sessions.find((row) => row.key === "agent:main:work")?.label).toBe(
      "Ops notes",
    );
  });

  it("does not persist autogenerated picker titles when rename is left unchanged", async () => {
    const { state } = createChatHeaderState();
    state.sessionKey = "agent:main:main";
    state.settings.sessionKey = state.sessionKey;
    const rows: GatewaySessionRow[] = [
      { key: "agent:main:main", kind: "direct", label: "Main chat", updatedAt: 6 },
      { key: "agent:main:work", kind: "direct", updatedAt: 5 },
    ];
    ...
    expect(state.chatSessionPickerEditingKey).toBeNull();
  });

  it("keeps the picker open while a rename save is in flight", async () => {
    const { state } = createChatHeaderState();
    state.sessionKey = "agent:main:main";
    state.settings.sessionKey = state.sessionKey;
    ...
    container
      .querySelector<HTMLButtonElement>('[data-chat-session-rename-save="agent:main:work"]')!
      .click();
    await vi.waitFor(() => expect(state.chatSessionPickerSavingKey).toBe("agent:main:work"));

    container
      .querySelector<HTMLElement>(".chat-session-picker")!
