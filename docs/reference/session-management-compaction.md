...
- **Control UI reconnect resume** can preserve the currently visible session for one `chat.send` when the client includes both the matching `sessionId` and `resumeSession: true`. Ordinary stale sends without this flag still create a new `sessionId`.
