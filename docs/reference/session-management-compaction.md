...
- `session.maintenance.modelRunPruneAfter`: short-lived gateway model-run probe retention (default `24h`; `false` disables). This applies only to strict explicit probe keys matching `agent:*:explicit:model-run-<uuid>` and runs before global stale-entry cleanup/capping.
...
separate `modelRunPruneAfter` retention only when their key exactly matches
`agent:*:explicit:model-run-<uuid>`; other explicit sessions are not part of
that 24h default. Isolated cron runs keep their own `cron.sessionRetention`
control, independent of model-run probe retention.
