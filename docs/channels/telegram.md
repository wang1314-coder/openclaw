...
- Long polling uses grammY runner with per-chat/per-thread sequencing. While a text-only turn is already in flight, later plain-text follow-ups in the same account/chat/thread use a narrow active-run ingress lane so they can reach the queue/steer policy without waiting for the running handler. Media and media-group updates remain on the normal serialized lane. Overall runner sink concurrency uses `agents.defaults.maxConcurrent`.
