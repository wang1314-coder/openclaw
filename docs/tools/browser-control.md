...
- `--labels` adds a screenshot with overlayed ref labels (prints `MEDIA:<path>`). The response also includes an `annotations` array with each ref's bounding box. On `screenshot`, `--labels` works with `--full-page`, `--ref`, and `--element`; on `snapshot`, the accompanying screenshot remains viewport-only.
...
- **Role snapshot (role refs like `e12`)**: `openclaw browser snapshot --interactive` (or `--compact`, `--depth`, `--selector`, `--frame`)
  ...
  - Add `--labels` to include a screenshot with overlayed `e12` labels and per-ref bounding-box metadata (`annotations[]`).
