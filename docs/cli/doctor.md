...
- Doctor checks exact linked ClawHub skill versions against ClawHub's public revocation verdict. A confirmed revoked version is reported; `doctor --fix` revalidates its origin, lock identity, and active install path before moving it under the workspace `.clawhub/quarantine/skills/` directory and removing it from the active lock. Lookup failures, offline runs, malformed responses, and identity mismatches never remove or move a skill.
