...
- For `parakeet-mlx`, if you pass `--output-dir`, OpenClaw reads `<output-dir>/<media-basename>.txt` only when `--output-format txt` is explicit; omitted or non-`txt` output formats fall back to stdout parsing.
