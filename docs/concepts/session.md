...
strict explicit keys like `agent:*:explicit:model-run-<uuid>` use
`session.maintenance.modelRunPruneAfter` (default `24h`) and are pruned before
the broader stale-entry age cutoff and entry cap. Set `modelRunPruneAfter: false`
to opt out; normal direct, group, thread, cron, hook, heartbeat, ACP, and
sub-agent sessions do not inherit this 24h retention.
