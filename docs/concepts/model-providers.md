...
- `openai/gpt-5.3-codex-spark` is available only through the Codex harness route (`api: "openai-chatgpt-responses"`) when the signed-in account exposes it; direct OpenAI API routes still suppress it because live API requests reject it
