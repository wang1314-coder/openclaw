...
- **`reset`**: primary reset policy. `daily` resets at `atHour` local time; `idle` resets after `idleMinutes`. When both configured, whichever expires first wins. Daily reset freshness uses the session row's `sessionStartedAt`; idle reset freshness uses `lastInteractionAt`. Background/system-event writes such as heartbeat, cron wakeups, exec notifications, and gateway bookkeeping can update `updatedAt`, but they do not keep daily/idle sessions fresh. The gateway maintenance sweep proactively rolls stale daily sessions after the boundary instead of waiting for the next inbound message. Provider-owned CLI sessions are exempt from the implicit daily default; configure `session.reset` explicitly or use `/reset` when CLI continuity should be cut on a timer.
