...
```json5
{
  session: {
    ...
    reset: {
      mode: "daily", // daily | idle | adaptive
      atHour: 4,
      idleMinutes: 60,
    },
    resetByType: {
...
- **`reset`**: primary reset policy. `daily` resets at `atHour` local time; `idle` resets after `idleMinutes`; `adaptive` waits for both the daily boundary and `idleMinutes`. With `daily` plus `idleMinutes`, whichever expires first wins. Daily/adaptive boundary freshness uses the session row's `sessionStartedAt`; idle/adaptive inactivity freshness uses `lastInteractionAt`. Background/system-event writes such as heartbeat, cron wakeups, exec notifications, and gateway bookkeeping can update `updatedAt`, but they do not keep sessions fresh.
