...
- `tools.fs.workspaceAliases`: optional exact aliases for `tools.fs.workspaceOnly=true`. Each entry maps a relative workspace `path` to an absolute host `target`; file tools must still use the workspace path, and direct target paths remain outside scope.
