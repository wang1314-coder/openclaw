...
- `gateway.auth.mode: "none"`: explicit no-auth mode. Use only for trusted local loopback setups; this is intentionally not offered by onboarding prompts. **Important**: When set to `none`, the gateway does not generate or provide a shared authentication token. Backend clients (such as embedded agent tools that connect via WebSocket to invoke gateway tools like `sessions_list`, `cron`, or `sessions_spawn`) will fail with `device identity required` errors because they cannot obtain a shared token for authentication. For backend client compatibility, use `gateway.auth.mode: "token"` instead.
