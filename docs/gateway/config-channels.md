...
**Opt-in stranded-reply recovery.** A capable model can still occasionally produce a substantive final without calling `message(action=send)` in tool-only mode (for example, after a long session it forgets the delivery step). By default that final stays private — no message call means no source reply, and nothing is auto-published. The gateway always records a diagnostic warning for such a stranded final, so the event remains observable in the verbose log even though no reply is delivered. Set `messages.strandedReplyRecovery: true` to opt into recovery: when a substantive tool-only final is stranded, OpenClaw enqueues a single follow-up turn asking the model to deliver the reply via `message(action=send)`. Recovery is bounded to one attempt (a retry that strands again is not retried, so there is no loop), and the private final text is replayed into the synthetic retry prompt with user-message persistence suppressed so it does not leak into durable session context. This flag is `false` by default; leaving it unset preserves the standard tool-only contract verbatim.
...
Fix: either pick a stronger tool-calling model, remove the explicit `"message_tool"` override to fall back to the `"automatic"` default, or set `messages.groupChat.visibleReplies: "automatic"` to force visible replies for every group/channel request. To keep tool-only mode but auto-recover the occasional stranded final, set `messages.strandedReplyRecovery: true` (see above) — the model is asked once to deliver via `message(action=send)` instead of the reply being silently dropped. The gateway hot-reloads `messages` config after the file is saved; only restart the gateway when file watching or config reload is disabled in the deployment.
