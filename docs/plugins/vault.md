...
The default resolver expects KV v2 mounted at `secret`. With those defaults,
this SecretRef id:
...
different Vault users at the same time, configure separate SecretRef providers
per tenant so each provider has its own auth environment.
