...
| `onCapabilities`   | No       | `Array<"provider" \| "channel" \| "tool" \| "hook">` | Broad capability hints used by control-plane activation planning. Prefer narrower fields when possible. Use `"hook"` when a plugin must be loaded into Gateway hook runtimes even though it does not own a provider, channel, or command surface. |
